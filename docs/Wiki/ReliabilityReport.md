...
- AOI / Inspector 로그 업로드 상한은 개발 환경과 EXE 런타임 모두 1GB로 고정합니다.
...
- 장시간 누적으로 커진 AOI / Inspector 로그도 파일 기반 후행 분석으로 다루며, 매우 큰 단일 로그는 청크 병렬 파싱으로 처리합니다.
...
- AOI 업로드 상한과 대용량 파싱 경로가 개발 환경 `.streamlit/config.toml`과 EXE 런타임 `run_app.py`에서 같은 값으로 유지돼야 운영/테스트 결과가 엇갈리지 않습니다.
