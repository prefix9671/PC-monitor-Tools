...
2. 버튼을 누른 뒤 초록색 성공 메시지와 파란 안내 메시지가 보이면 UI 기준으로는 정상 시작입니다.
...
1. 왼쪽 사이드바를 아래로 내려 `AOI / 인스펙터 로그` 구역을 찾습니다.
2. `Browse files`를 눌러 TXT 또는 LOG 파일을 선택합니다.
3. 파일명과 초록색 불러오기 성공 메시지가 보이면 업로드가 정상 완료된 상태입니다.
4. 경로를 직접 입력해야 할 때만 아래 `고급: 경로로 AOI / 인스펙터 로그 불러오기`를 사용합니다.
...
- 다만 `NO` 값 자체는 시간 필터 후 다시 매기지 않고, 원본 AOI 로그 기준 번호를 그대로 유지합니다.
...
1. AOI 로그를 불러오면 대시보드 아래에 `검사 결과 XLSX 내보내기` 패널이 생깁니다.
...
1. `메모리 + 인스펙터 대시보드`는 OS 메모리와 AOI 로그 메모리를 함께 보는 화면입니다.
